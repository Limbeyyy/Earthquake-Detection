...
def transform_embedding(features, params):
    x = keras.layers.Dense(256, activation=config.ACTIVATION, kernel_regularizer=l2(L2_REG))(features)
    x = Dropout(0.4)(x)
...
def Contrastive_Network_dual(input_wave_size, input_spec_shape, embedding_model):
    inputs = {}
    for name in ["refer", "positive", "negative"]:
    ...
    return keras.Model(inputs=list(inputs.values()),
                       outputs=[emb["refer"], emb["positive"], emb["negative"]])

# ---------------- Contrastive Model (Triplet Only) ----------------
class Contrastive_Model(keras.Model):
    def __init__(self, Contrastive_network, batch_size, loss_tracker, feature_distance,
                 metric_acc=None, margin=1):
        super().__init__()
    ...
    def _compute_distance(self, inputs):
        emb_ref, emb_pos, emb_neg = self.Contrastive_network(inputs)
        return self.distance_func(emb_ref, emb_pos), self.distance_func(emb_ref, emb_neg)

    def _compute_loss(self, pos, neg):
        return tf.maximum(pos - neg + self.margin, 0.0)

    def _compute_acc(self, pos, neg):
        y_pred = pos < neg
        if self.metric_acc:
    ...
    def train_step(self, inputs):
        with tf.GradientTape() as tape:
            pos, neg = self._compute_distance(inputs)
